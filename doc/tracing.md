# User-space, Statically Defined Tracing (USDT) for Garikcoin Core

Garikcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Garikcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Garikcoin Core
...
To list probes in Garikcoin Core, use `info probes` in `gdb`:
...
The `readelf` tool can be used to display the USDT tracepoints in Garikcoin Core.
