Garikcoin Core version 0.13.0 is now available from:
...
Various code modernizations have been done. The Garikcoin Core code base has
...
HD wallets are incompatible with older versions of Garikcoin Core.
...
proposed parameters, a future release of Garikcoin Core is expected that
...
The two phases are distinct in the Garikcoin-Qt GUI. During the first one,
...
- #8207 `f7a403b` Add a link to the Garikcoin-Core repository and website to the About Dialog (MarcoFalke)
...
- #8164 `0f24eaf` [Garikcoin-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
