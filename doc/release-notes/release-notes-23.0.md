...
Garikcoin Core version 23.0 is now available from:
...
Garikcoin Core release binaries for Linux now include experimental tracepoints which
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Garikcoin Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Garikcoin Core in 22.0. (#22544)
