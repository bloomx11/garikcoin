...
- [Development guidelines](#development-guidelines)
    - [General Garikcoin Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Garikcoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Garikcoin Core is a multi-threaded application, and deadlocks or other
...
Garikcoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Garikcoin Core, where everyone uses
...
pay attention to for reviewers of Garikcoin Core code.

General Garikcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Garikcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Garikcoin Core, in which case
...
a Garikcoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
file descriptors, it will cause problems with Garikcoin's `select()` loop, because
...
compatibility between nodes. This happened in Garikcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Garikcoin's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Garikcoin Core, the wallet was always
