# CJDNS support in Garikcoin Core

It is possible to run Garikcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Garikcoin Core, similarly to a VPN (either in the host/OS or on
...
before it will work with your Garikcoin Core node. This is described in steps
...
## Run Garikcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Garikcoin Core
...
When enabled, this option tells Garikcoin Core that it is running in an
...
IPv6 local network. This helps Garikcoin Core perform better address management:
...
CJDNS support was added to Garikcoin Core in version 23.0 and there may be fewer
...
## CJDNS-related information in Garikcoin Core

There are several ways to see your CJDNS address in Garikcoin Core:
