...
These tools must be installed in order to build Garikcoin Core from source.
...
Garikcoin Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Garikcoin Core application using `make deploy`.
...
Run the following in your terminal to compile Garikcoin Core:
...
## Running Garikcoin Core

Garikcoin Core should now be available at `./src/bitcoind`.
