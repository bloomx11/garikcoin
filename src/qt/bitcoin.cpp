...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<GarikcoinUnit>("GarikcoinUnit");
#else
    qRegisterMetaType<GarikcoinUnit>("GarikcoinUnit");
#endif
}
...
GarikcoinApplication::GarikcoinApplication():
...
void GarikcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the GarikcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", GarikcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void GarikcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool GarikcoinApplication::createOptionsModel(bool resetSettings)
{
...
void GarikcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new GarikcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &GarikcoinGUI::quitRequested, this, &GarikcoinApplication::requestShutdown);
...
void GarikcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
    ...
    m_splash->show();
    connect(this, &GarikcoinApplication::splashFinished, m_splash, &SplashScreen::finish);
    connect(this, &GarikcoinApplication::requestedShutdown, m_splash, &QWidget::close);
}

void GarikcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool GarikcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void GarikcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &GarikcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &GarikcoinApplication::handleRunawayException);
    connect(this, &GarikcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &GarikcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void GarikcoinApplication::parameterSetup()
{
...
void GarikcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void GarikcoinApplication::requestInitialize()
{
...
void GarikcoinApplication::requestShutdown()
{
...
void GarikcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &GarikcoinGUI::handlePaymentRequest);
            connect(window, &GarikcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void GarikcoinApplication::handleRunawayException(const QString &message)
{
...
void GarikcoinApplication::handleNonFatalException(const QString& message)
{
...
WId GarikcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool GarikcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", GarikcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
