...
class GarikcoinTestMetaClass(type):
    """Metaclass for GarikcoinTestFramework.

    Ensures that any attempt to register a subclass of `GarikcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'GarikcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("GarikcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("GarikcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class GarikcoinTestFramework(metaclass=GarikcoinTestMetaClass):
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("GarikcoinRPC")
